...
const PolicyForm = () => {
    ...
    const handleOtherBenChange = (index, value) => {
        ...
        // Update formData based on index
        if (index === 0) {
            setFormData((prevData) => ({
                ...prevData,
                benefit1: value,
    ...
    const addOtherBenefit = () => {
        // Limit creation to two other benefits
        if (otherBenefits.length < 2) {
    ...
    const handleKeyDetailsChange = (index, value) => {
        ...
        console.log(concatenatedKeyDetails);
        setFormData((prevData)=>({
            ...prevData,
            details: concatenatedKeyDetails,
        }));
    };
    ...
    const handleBenefitChange = (index, field, inputValue) => {
        ...
        // Convert updatedBenefit array to a concatenated string
        const benefitsString = updatedBenefit.map((item) => `${item.label} \n ${item.value}`).join('    \n\n    ');
